...
A critical constraint: exported CSV files reside on the internal DSpace server filesystem. To share them with other agents (e.g., `minio_agent`), the `github_agent` must first copy the file to the shared `DOWNLOADS_DIR`.
